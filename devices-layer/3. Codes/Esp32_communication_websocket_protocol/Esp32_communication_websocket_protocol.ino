...
WebSocketsServer webSocket(81); // WebSocket port
  ...
  if (type == WStype_TEXT) {
    ...
    if (doc.containsKey("temp2") && doc.containsKey("temp3") &&
      ...
      if (t2 > 0 && t3 > 0 && t2 < t3 && fan >= 0 && fan <= 3) {
        ...
        isManual = manual;
        fanLevel = fan;
        Serial.println("✅ Dữ liệu cập nhật từ WebSocket");
        sendStatus();
      } else {
...
void handleButtonMode() {
  if (digitalRead(BUTTON_Mode) == LOW && prevMode == HIGH) {
    ...
    Serial.println(displayMode == 0 ? ">> Hiển thị nhiệt độ + ánh sáng" : ">> Hiển thị Temp2 - Temp3");
    prevMode = LOW;
...
void handleButtonFan() {
  ...
  if (!isPressed && buttonHeld) {
    ...
    if (pressTime >= 2000) {
      ...
      Serial.println(isManual ? ">> Bật chế độ quạt thủ công" : ">> Trở lại chế độ tự động");
    } else if (isManual) {
